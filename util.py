...
def _find_es_dict_by_key(lookup_dict, term):
    ...
    """
    dict_term=("48:lookup_dict:%s:term:%s:" % (lookup_dict,term))
   # logging.warning(dict_term)
...
def lookup_es_key(lookup_dict, term):
    ...
    """
    value_dict, value_key = _find_es_dict_by_key(lookup_dict, term)
    value_key_106=("106:value_dict:%s:value_key:%s" % (value_dict,value_key))
   # logging.warning(value_key_106)
...
def pretty_ts_local(timestamp, tz=True):      #create by 2017/07/12
...
def ts_now():
   ...
   # now1 = now.astimezone(dateutil.tz.tzlocal())
   # return now1
...
def format_index(index, start, end):
    ...
    while start.date() <= end.date():
       # indexes.append(start.strftime(index))
        start_local_timezone = pretty_ts_local(start)
        indexes.append(start_local_timezone.strftime(index))
